...
function drawSparkline(canvas, values, color) {
  ...
  // Downsample to ~120 pts for readability
  let pts = values;
  if (pts.length > 120) {
    const step = pts.length / 120;
    pts = Array.from({ length: 120 }, (_, i) => pts[Math.floor(i * step)]);
  }
...
function renderAnalysis(container, result, sensorType) {
  ...
  result.analyses.slice(0, 4).forEach(a => {
    ...
    drawSparkline(wrap.querySelector('canvas'), a.values, cfg.color);
  });
}
