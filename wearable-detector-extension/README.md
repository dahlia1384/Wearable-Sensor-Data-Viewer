...
**MutationObserver**
A browser API that watches for changes to the page's DOM (HTML structure). The extension uses it to detect when GitHub's single-page app navigates to a new repository without reloading the page, so it can re-run the file scan.
